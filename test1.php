...
echo "=== DEFINITIVE TEST: Are PDO Queries Cooperative(using Pool Class)? ===\n\n";

$pool = new Pool($config, 20);

// Setup
await(async(function() use ($pool) {
    $pdo = await($pool->get());
    $pdo->exec("CREATE TABLE IF NOT EXISTS pool_test (
        id INT AUTO_INCREMENT PRIMARY KEY,
        data VARCHAR(255)
    )");
    $pdo->exec("TRUNCATE TABLE pool_test");
...
for ($i = 1; $i <= 5; $i++) {
    $promises[] = async(function() use ($pool, $i, $start) {
        $startTime = microtime(true);
        echo "[{$i}] 🚀 STARTED at " . round(($startTime - $start) * 1000) . "ms\n";
        
        $pdo = await($pool->get());
        $gotConnectionTime = microtime(true);
        echo "[{$i}] 🔌 Got connection at " . round(($gotConnectionTime - $start) * 1000) . "ms\n";
...
for ($i = 1; $i <= 3; $i++) {
    $promises[] = async(function() use ($pool, $i, $start, &$timeline) {
        $events = [];
        
        $events['fiber_start'] = microtime(true) - $start;
        
        $pdo = await($pool->get());
        $events['got_connection'] = microtime(true) - $start;
        ...
        $pool->release($pdo);
        $events['released'] = microtime(true) - $start;
...
await(async(function() use ($pool) {
    $pdo = await($pool->get());
    $pdo->exec("DROP TABLE IF EXISTS pool_test");
    $pool->release($pdo);
}));
