...
describe('AsyncPDO Stress Test - PostgreSQL', function () {
    beforeEach(function () {
        ...
        StressTestHelper::setupPostgreSQLSchema()->await();
    });

    afterEach(function () {
        Task::run(function () {
            await(AsyncPDO::execute('DROP TABLE IF EXISTS order_items CASCADE'));
            await(AsyncPDO::execute('DROP TABLE IF EXISTS orders CASCADE'));
            await(AsyncPDO::execute('DROP TABLE IF EXISTS users CASCADE'));
            await(AsyncPDO::execute('DROP TYPE IF EXISTS order_status CASCADE'));
        });
        AsyncPDO::reset();
    });
